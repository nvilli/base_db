...
class TSNDataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #### permute sampled frame index
        _index, permute_label = self.permute_index(_index)
        ...
        return index, data, permute_label

    def permute_index(self, index):

        label = random.randint(0, 23)
        _permutelist = self.permute_list[label]

        _index = []
        for i in range(len(_permutelist)):
            for j in range(4):
                _index.append(_permutelist[i] + j)
